...
interface EmployeesTableContextProps {
  featureFlags: FeatureFlagValue[];
  setFeatureFlags: React.Dispatch<React.SetStateAction<FeatureFlagValue[]>>;
}

const EmployeesTableContext = React.createContext<EmployeesTableContextProps>({
  featureFlags: [],
  setFeatureFlags: () => {},
});

export function useEmployeesTable() {
  const context = React.useContext(EmployeesTableContext);
  if (!context) {
    throw new Error(
...
export function EmployeesTableProvider({ children }: React.PropsWithChildren) {
